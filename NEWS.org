...
The big story for this version is to re-implement the excerpt processing.  The implementation chosen uses a temporary buffer and Emacs text editing functions, building on top of Emacs text processing awareness.  As Emacs grows smarter, so will MoC.
** Added ❇️
- ~moc-focus-playback-version~ to make it possible to compare the playback version of a playback expression
** Changed 🔮
